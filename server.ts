...
async function ytFetch(endpoint: string, params: Record<string, string>) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`YouTube API Error: ${res.status} ${res.statusText} - ${err.error?.message || ''}`);
  }
  return res.json();
}
...
async function fetchVideoStats(videoIds: string[], shortsSet: Set<string> = new Set()) {
  const stats: any[] = [];
  // YouTube API allows maximum 50 IDs per request for /videos
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const results = (data.items || []).map((item: any) => ({
      ...
      handle: '' // Basic search doesn't return handles easily without extra fetch
    }));
    res.json(results);
  } catch (error: any) {
    ...
    for (const input of inputs) {
      if (input.type === 'video') {
        ...
        if (pData.items && pData.items.length > 0) {
          ...
          const videos = await fetchAllVideosFromPlaylist(input.id, shortsSet, 40); // User rules: top 40 for playlists
        ...
        if (input.type === 'channel') {
          ...
          params.forHandle = input.handle; // e.g. @handle (API supports forHandle)
        }
        ...
        if (cData.items && cData.items.length > 0) {
          ...
          const bannerUrl = item.snippet.thumbnails?.high?.url || ''; // Approximation if brandingSettings isn't requested (to save quota)
