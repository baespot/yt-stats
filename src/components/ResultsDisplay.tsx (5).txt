...
const VirtualTable = ({
  ...
  const totalRows = sortedVideos.length;
  ...
  const startIndex = startRowIndex;
  const visibleRows = viewMode === 'list'
    ? sortedVideos.slice(startIndex, endRowIndex)
    : sortedVideos; // grid: render all, natural height
  const topSpacerHeight = viewMode === 'list' ? startRowIndex * rowHeight : 0;
  const bottomSpacerHeight = viewMode === 'list' ? Math.max(0, (totalRows - endRowIndex) * rowHeight) : 0;
        ...
        <div className="overflow-y-auto custom-scrollbar w-full relative" style={{ height: '600px' }} ref={containerRef}>
          ...
          {viewMode === 'list' && (
            <table className="w-full text-left text-sm m-0 border-spacing-0 table-fixed">
              ...
              <tbody className="bg-white">
                {topSpacerHeight > 0 && <tr style={{ height: topSpacerHeight }}><td colSpan={4} /></tr>}
                {visibleRows.map((video, idx) => {
                  ...
                  return (
                    <tr key={video.id + idx} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0" style={{ height: LIST_ROW_HEIGHT }}>
                      ...
                      {/* Title + Date + optional channel as sub-line */}
                      <td className="px-2 py-2 align-middle" style={{ overflow: 'hidden', maxWidth: 0 }}>
                        ...
                        <p className="text-[9px] font-mono text-[var(--text-tertiary)] mt-0.5 overflow-hidden w-full">
                          <span className="block overflow-hidden text-ellipsis whitespace-nowrap">
                            {new Date(video.uploadDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                          </span>
                          <span className="block overflow-hidden text-ellipsis whitespace-nowrap">
                            {new Date(video.uploadDate).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}
                            {showChannel && video.channelName ? ` · ${video.channelName}` : ''}
                          </span>
                        </p>
                      </td>
                      {/* Views + Likes stacked right-aligned */}
                ...
                {bottomSpacerHeight > 0 && <tr style={{ height: bottomSpacerHeight }}><td colSpan={4} /></tr>}
              </tbody>
            </table>
          )}
          ...
          {viewMode === 'grid' && (
            <div className="flex flex-col relative w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 p-3 md:p-4">
                {visibleRows.map((video, idx) => {
                  const rank = startIndex + idx + 1;
                  return (
                    <div key={video.id + idx} className="flex flex-col bg-white border border-gray-100 hover:border-[var(--text-tertiary)] transition-colors rounded-sm overflow-hidden">
                      ...
                      <div className="p-2 flex flex-col">
                        ...
                        <p className="text-[9px] font-mono text-[var(--text-tertiary)] truncate">
                          {new Date(video.uploadDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                        </p>
                        <p className="text-[9px] font-mono text-[var(--text-tertiary)] truncate mb-1">
                          {new Date(video.uploadDate).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}
                          {showChannel && video.channelName ? ` · ${video.channelName}` : ''}
                        </p>
...
const ChannelCard: React.FC<{ channel: ChannelStats, defaultOpen?: boolean, includeShorts: boolean }> = ({ channel, defaultOpen = true, includeShorts }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 overflow-hidden mb-12 fade-in">
      {/* Channel banner: natural size, no forced ratio, no crop */}
      {channel.bannerUrl && (
...
const ChannelLeaderboard = ({ channels, includeShorts }: { channels: ChannelStats[], includeShorts: boolean }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 overflow-hidden mt-6 mb-12 w-full fade-in">
      <div className="flex flex-row items-center justify-between p-4 border-b border-gray-100 bg-white gap-4">
        <h3 className="section-heading" style={{ fontSize: '15px' }}>CHANNEL LEADERBOARD</h3>
        <button onClick={exportCSV} className="px-3 py-1.5 text-xs font-mono font-bold uppercase tracking-wider text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-gray-50 transition-colors border border-gray-100 min-h-[44px] shrink-0">CSV</button>
      </div>
      <div className="overflow-x-auto w-full">
        <table className="w-full text-left text-sm m-0">
          <thead className="bg-white text-[var(--text-secondary)] text-[10px] uppercase tracking-widest font-mono font-bold">
            <tr>
              <th className="px-2 py-3 text-center border-b border-gray-100 w-9">#</th>
              ...
              <th className="px-2 py-3 text-right border-b border-gray-100 whitespace-nowrap">Videos</th>
            </tr>
          </thead>
          <tbody className="bg-white">
            {sorted.map((c, idx) => {
              ...
              return (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                  ...
                  <td className="px-3 py-3 font-bold text-[var(--text-primary)] tracking-tight whitespace-nowrap">
