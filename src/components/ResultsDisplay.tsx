...
// Format date + time: "Mar 31, 2024 · 9:00 AM"
...
const VirtualTable = ({ videos, showChannel = false, title = "Top Videos", isCollapsible = false, isOpen = true, onToggle = () => {} }: {
  ...
  const sortLabel: Record<SortOption, string> = {
    views: 'Most Popular', likes: 'Most Liked', recent: 'Most Recent', oldest: 'Oldest', title: 'Title A-Z'
  };
          ...
          {viewMode === 'list' && (
            <table className="w-full text-left text-sm m-0 border-spacing-0 table-fixed">
              <colgroup>
                ...
                <col style={{ width: '60px' }} />        {/* Thumb */}
                <col />                                  {/* Title/Date — takes remaining space */}
                {showChannel && <col style={{ width: '90px' }} />}  {/* Channel */}
                <col style={{ width: '110px' }} />       {/* Views/Likes */}
              </colgroup>
              <thead className="bg-white text-[var(--text-secondary)] text-[10px] uppercase tracking-widest font-mono font-bold sticky top-0 z-10">
                <tr>
                  ...
                  {showChannel && <th className="px-2 py-2 border-b border-gray-100">Channel</th>}
                  ...
                  return (
                    <tr key={video.id + idx} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0" style={{ height: LIST_ROW_HEIGHT }}>
                      ...
                      <td className="px-1 py-2 text-center align-middle">
                        <span className="inline-flex items-center justify-center w-7 h-5 rounded-full font-mono font-bold text-[10px]" style={getRankStyle(rank)}>
                          {rank}
                        </span>
                      </td>
                      {/* Thumbnail */}
                      <td className="px-1 py-2 align-middle">
                        <a href={`https://youtube.com/watch?v=${video.id}`} target="_blank" rel="noreferrer" className="block shrink-0">
                          <img
                            src={`https://i.ytimg.com/vi/${video.id}/mqdefault.jpg`}
                            className="rounded object-cover shrink-0"
                            style={{ width: '52px', aspectRatio: '16/9', border: '1px solid var(--border-color)' }}
                            alt="" loading="lazy"
                          />
                        </a>
                      </td>
                      {/* Title + Date */}
                      <td className="px-2 py-2 align-middle" style={{ overflow: 'hidden' }}>
                        <a
                          href={`https://youtube.com/watch?v=${video.id}`}
                          target="_blank" rel="noreferrer"
                          className="font-bold text-[var(--text-primary)] hover:text-[var(--accent)] transition-colors text-xs leading-snug block"
                          style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}
                          title={video.title}
                        >
                          {video.title}
                        </a>
                        <p className="text-[10px] font-mono text-[var(--text-tertiary)] mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
                      ...
                      {showChannel && (
                        <td className="px-2 py-2 align-middle" style={{ overflow: 'hidden' }}>
                          <span className="text-[10px] font-mono text-[var(--text-secondary)] block overflow-hidden text-ellipsis whitespace-nowrap">{video.channelName}</span>
                        </td>
                      )}
                      {/* Views + Likes stacked */}
                      <td className="px-2 py-2 text-right align-middle">
                        <span className="font-mono font-bold text-[var(--text-primary)] text-xs block whitespace-nowrap">{formatNumber(video.views)}</span>
                        <span className="font-mono text-[var(--text-tertiary)] text-[10px] block whitespace-nowrap">♥ {formatNumber(video.likes)}</span>
                      </td>
                    </tr>
                  );
                })}
...
// StatCard: use min font size + no truncation so full numbers always show on desktop
const StatCard = ({ label, value, subLabel }: { label: string, value: string | number, subLabel?: React.ReactNode }) => (
  <div className="flex flex-col p-4 md:p-6 bg-white border border-gray-100 w-full">
    ...
    {subLabel && (
      <span className="text-[10px] md:text-xs text-[var(--text-tertiary)] mt-2 font-mono uppercase tracking-widest" style={{ wordBreak: 'break-word' }}>
        {subLabel}
      </span>
    )}
  </div>
...
const ChannelCard: React.FC<{ channel: ChannelStats, defaultOpen?: boolean, includeShorts: boolean }> = ({ channel, defaultOpen = true, includeShorts }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 overflow-hidden mb-12 fade-in">
      {/* Banner at natural aspect ratio, not cropped */}
      {channel.bannerUrl && (
        <img src={channel.bannerUrl} alt="" className="w-full block" style={{ display: 'block' }} />
      )}
...
const PlaylistCard: React.FC<{ playlist: PlaylistStats, includeShorts: boolean }> = ({ playlist, includeShorts }) => {
  ...
  // Thumbnail = latest video's thumbnail at natural ratio
  ...
  return (
    <div className="bg-white border border-gray-100 overflow-hidden mb-12 fade-in">
      {thumbUrl && (
        <img src={thumbUrl} alt="" className="w-full block" />
      )}
      <div className="p-4 md:p-8">
    ...
    <div className="bg-white border border-gray-100 overflow-hidden mt-6 mb-12 w-full fade-in">
      ...
      <table className="w-full text-left text-sm m-0 table-fixed">
        ...
        <thead className="bg-white text-[var(--text-secondary)] text-[10px] uppercase tracking-widest font-mono font-bold">
          <tr>
            <th className="px-3 py-3 text-center border-b border-gray-100">#</th>
            <th className="px-3 py-3 border-b border-gray-100">Channel</th>
            <th className="px-3 py-3 text-right border-b border-gray-100">Likes</th>
            <th className="px-3 py-3 text-right border-b border-gray-100">Views</th>
            <th className="px-3 py-3 text-right border-b border-gray-100">Videos</th>
          </tr>
        </thead>
        <tbody className="bg-white">
          {sorted.map((c, idx) => {
            ...
            return (
              <tr key={c.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                ...
                <td className="px-3 py-3 font-bold text-[var(--text-primary)] tracking-tight overflow-hidden">
                  <a href={`https://youtube.com/channel/${c.id}`} target="_blank" rel="noreferrer" className="hover:text-[var(--accent)] transition-colors block overflow-hidden text-ellipsis whitespace-nowrap">
                    {c.name}
                  </a>
                </td>
                <td className="px-3 py-3 text-right font-mono font-bold text-[var(--text-primary)] text-xs whitespace-nowrap">{formatNumber(c.displayLikes)}</td>
                <td className="px-3 py-3 text-right font-mono font-bold text-[var(--text-primary)] text-xs whitespace-nowrap">{formatNumber(c.displayViews)}</td>
                <td className="px-3 py-3 text-right font-mono font-bold text-[var(--text-primary)] text-xs whitespace-nowrap">{formatNumber(c.displayVideoCount)}</td>
              </tr>
            );
          })}
        </tbody>
...
export const ResultsDisplay = ({ data }: { data: AnalysisResponse }) => {
  ...
  if (showMultiChannel) {
    ...
    combinedVideos = combinedVideos.slice(0, 50);
  }
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-px bg-[var(--border-color)] border border-gray-100">
                    <div className="bg-white p-2 flex flex-col overflow-hidden">
                      ...
                      <p className="font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(v.views)}</p>
                    </div>
                    <div className="bg-white p-2 flex flex-col items-end overflow-hidden">
                      ...
                      <p className="font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(v.likes)}</p>
                    </div>
                    <div className="bg-white p-2 flex flex-col overflow-hidden">
                      <p className="text-[9px] uppercase tracking-widest font-mono text-[var(--text-secondary)] whitespace-nowrap">Ratio</p>
                      <p className="font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{v.likeViewRatio}%</p>
                    </div>
                    <div className="bg-white p-2 flex flex-col items-end overflow-hidden">
