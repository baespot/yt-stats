    ...
    <div className="bg-white border border-gray-100 overflow-hidden mt-6 flex flex-col w-full">
      ...
      {isOpen && (
        <div className="overflow-x-auto overflow-y-auto custom-scrollbar w-full relative" style={{ height: '600px' }} ref={containerRef}>
          {viewMode === 'list' && (
            <table className="w-full text-left text-sm m-0 border-spacing-0">
              <thead className="bg-white text-[var(--text-secondary)] text-xs uppercase tracking-widest font-mono font-bold sticky top-0 z-10 box-shadow-[0_1px_0_var(--border-color)]">
                <tr>
                  <th className="px-2 py-3 w-10 border-b border-gray-100 whitespace-nowrap">Rank</th>
                  <th className="px-2 py-3 border-b border-gray-100">Video</th>
                  {showChannel && <th className="px-2 py-3 border-b border-gray-100 whitespace-nowrap">Channel</th>}
                  <th className="px-2 py-3 text-right border-b border-gray-100 whitespace-nowrap">Views</th>
                  <th className="px-2 py-3 text-right border-b border-gray-100 whitespace-nowrap">Likes</th>
                  <th className="px-2 py-3 border-b border-gray-100 text-right whitespace-nowrap">Date</th>
                </tr>
              </thead>
                    ...
                    <tr key={video.id + idx} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0" style={{ height: LIST_ROW_HEIGHT }}>
                      ...
                      <td className="px-2 py-2 max-w-0 w-full">
                        <div className="flex items-center gap-2">
                          <img src={`https://i.ytimg.com/vi/${video.id}/mqdefault.jpg`} className="w-14 rounded aspect-video object-cover shrink-0 block" style={{border:"1px solid var(--border-color)"}} alt="" loading="lazy" />
                          <a href={`https://youtube.com/watch?v=${video.id}`} target="_blank" rel="noreferrer" className="text-[var(--text-primary)] hover:text-[var(--accent)] transition-colors font-bold tracking-tight block leading-snug overflow-hidden" style={{display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>
                            {video.title}
                          </a>
                        </div>
                      </td>
                      {showChannel && <td className="px-2 py-2 text-[var(--text-secondary)] font-mono text-xs whitespace-nowrap max-w-[100px] overflow-hidden text-ellipsis">{video.channelName}</td>}
                      <td className="px-2 py-2 text-right font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(video.views)}</td>
                      <td className="px-2 py-2 text-right font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(video.likes)}</td>
                      <td className="px-2 py-2 font-mono text-[var(--text-tertiary)] text-[10px] text-right whitespace-nowrap tracking-widest">{new Date(video.uploadDate).toLocaleDateString()}</td>
                    </tr>
                  );
                })}
...
const StatCard = ({ label, value, subLabel, isRanked = false }: { label: string, value: string | number, subLabel?: React.ReactNode, isRanked?: boolean }) => (
  <div className="flex flex-col p-4 md:p-6 bg-white border border-gray-100 w-full overflow-hidden">
    <span className="text-[10px] md:text-xs font-light tracking-[0.1em] uppercase text-[var(--text-secondary)] mb-2 truncate">{label}</span>
    <span className="font-mono font-bold text-[var(--text-primary)] leading-none truncate" style={{ fontSize: 'clamp(0.85rem, 2.5vw, 1.6rem)' }}>
      {value}
    </span>
    {subLabel && <span className="text-[10px] md:text-xs text-[var(--text-tertiary)] mt-2 font-mono truncate uppercase tracking-widest">{subLabel}</span>}
  </div>
);
...
const PlaylistCard: React.FC<{ playlist: PlaylistStats, includeShorts: boolean }> = ({ playlist, includeShorts }) => {
  ...
  const thumbUrl = latestVideo ? `https://i.ytimg.com/vi/${latestVideo.id}/mqdefault.jpg` : playlist.thumbnailUrl;

  return (
    <div className="bg-white border border-gray-100 overflow-hidden mb-12 fade-in">
      {thumbUrl && (
        <div className="h-32 md:h-48 w-full bg-cover bg-center overflow-hidden bg-black" style={{ backgroundImage: `url(${thumbUrl})` }} />
      )}
      <div className="p-4 md:p-8">
      ...
      <div className="overflow-x-auto w-full custom-scrollbar">
        <table className="w-full text-left text-sm m-0">
          <thead className="bg-white text-[var(--text-secondary)] text-xs uppercase tracking-widest font-mono font-bold">
            <tr>
              <th className="px-3 py-3 w-14 border-b border-gray-100 whitespace-nowrap">Rank</th>
              <th className="px-3 py-3 border-b border-gray-100">Channel Name</th>
              <th className="px-3 py-3 text-right border-b border-gray-100 whitespace-nowrap">Likes</th>
              <th className="px-3 py-3 text-right border-b border-gray-100 whitespace-nowrap">Views</th>
              <th className="px-3 py-3 text-right border-b border-gray-100 whitespace-nowrap">Videos</th>
            </tr>
          </thead>
          <tbody className="bg-white">
            {sorted.map((c, idx) => {
              ...
              return (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                  ...
                  <td className="px-3 py-3 font-bold text-[var(--text-primary)] tracking-tight">
                    <a href={`https://youtube.com/channel/${c.id}`} target="_blank" rel="noreferrer" className="hover:text-[var(--accent)] transition-colors">
                      {c.name}
                    </a>
                  </td>
                  <td className="px-3 py-3 text-right font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(c.displayLikes)}</td>
                  <td className="px-3 py-3 text-right font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(c.displayViews)}</td>
                  <td className="px-3 py-3 text-right font-mono font-bold text-[var(--text-primary)] whitespace-nowrap">{formatNumber(c.displayVideoCount)}</td>
                </tr>
              );
            })}
          </tbody>
